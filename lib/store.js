...
class Store {
  constructor(driver, keyWhiteList) {
    ...
    this._purgeableKeys = [];
    this._maxRetryCount = MAX_RETRY_COUNT;
  ...
  setItem(key, value, callback) {
    ...
    return this._setItemWithRetry(key, value, callback);
  }

  _setItemWithRetry(key, value, callback, retryCount = 0) {
    return this._driver.setItem(key, value).then(function () {
      ...
      if (retryCount >= this._maxRetryCount) {
        const e = new Error('exceeded max retry count');
        if (callback) {
          callback(e);
        }
        return Promise.reject(e);
      }
      return this._performRecovery().then(function () {
        return this._setItemWithRetry(key, value, callback, retryCount + 1);
      }.bind(this), function (/* recoveryError */) {
        if (callback) {
          callback(error);
        }
  ...
  setPurgeableItem(key, value, callback) {
    ...
    return this._setPurgeableItemWithRetry(key, value, callback);
  }

  _setPurgeableItemWithRetry(key, value, callback, retryCount = 0) {
    this._purgeableKeys = this._maintainLRUOrder(
    ...
    return this.multiSetTransactionally(keyValuePairs).then(function () {
      ...
      return this._performRecovery().then(function () {
        return this._setPurgeableItemWithRetry(
          key,
          value,
          callback,
          retryCount + 1
