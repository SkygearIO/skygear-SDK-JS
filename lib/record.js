...
export default class Record {
  ...
  addReadAccess(role) {
    this.access.addReadAccess(role);
  }

  removeReadAccess(role) {
    this.access.removeReadAccess(role);
  }

  addWriteAccess(role) {
    this.access.addWriteAccess(role);
  }

  removeWriteAccess(role) {
    this.access.removeWriteAccess(role);
  }

  addPublicReadAccess() {
    this.access.addPublicReadAccess();
  }

  removePublicReadAccess() {
    this.access.removePublicReadAccess();
  }

  addPublicWriteAccess() {
    this.access.addPublicWriteAccess();
  }

  removePublicWriteAccess() {
    this.access.removePublicWriteAccess();
  }
