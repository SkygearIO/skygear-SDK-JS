...
const _metaAttrs = {
  ...
  _default_access: { //eslint-disable-line
    parser: (v) => {
      let acl = v;
      if (v && v.toJSON) {
        acl = v.toJSON();
...
export default class Record {
  ...
  constructor(recordType, attrs = defaultAttrs) {
    ...
    this._id = id;
    this._access = null;
    this._defaultAccess = Record.defaultACL;
    this.update(attrs);
  ...
  setDefaultAccess(acl) {
    this._defaultAccess = acl;
  }
  ...
  toJSON() {
    let payload = {
      ...
      _default_access: this._defaultAccess.toJSON() //eslint-disable-line
    };
    _.each(this.attributeKeys, function (key) {
