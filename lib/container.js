  ...
  makeRequest(action, data) {
    ...
    return new Promise(function (resolve, reject) {
      _request.end(function (err, res) {
        // Do an application JSON parse beacuse in some condition, the content-
        // type header will got strip and it will not deserial the json for us.
        let body = res.body;
        if (body === null) {
          body = JSON.parse(res.text);
        }
        if (err) {
          let skyErr = body.error || err.response.text;
          reject({
            status: err.status,
            error: skyErr
          });
