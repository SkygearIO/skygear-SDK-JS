...
export async function oauthHandler() {
  const data = await this.container.makeRequest('_auth/sso/config');
...
export async function unlinkOAuthProvider(provider) {
  return this.container.makeRequest(`_auth/sso/${provider}/unlink`);
}
...
export async function getOAuthProviderProfiles() {
  const data = await this.container.makeRequest('_auth/sso/provider_profiles');
...
async function _getRedirectResult(action, resolvePromise) {
  ...
  try {
    ...
    oauthIframe.src = this.container.url + '_auth/sso/iframe_handler';
...
function _getAuthURL(provider, action) {
  return {
    login: `_auth/sso/${provider}/login_auth_url`,
    link: `_auth/sso/${provider}/link_auth_url`
  }[action];
}

function _getAuthWithAccessTokenURL(provider, action) {
  return {
    login: `_auth/sso/${provider}/login`,
    link: `_auth/sso/${provider}/link`
  }[action];
}
