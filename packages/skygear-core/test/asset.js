...
describe('Asset Signer', function () {
  describe('File System Signer', function () {
    ...
    it('creates with settings', async function () {
    ...
    it('signs url', async function () {
      const clock = sinon.useFakeTimers(1481095934000);
      const signer = new FSSigner(options);
      const signed = await signer.sign('index.html');
      clock.restore();
      expect(signed).to.equal('http://skygear.dev/files/index.html'
  ...
  describe('S3 Signer', function () {
    ...
    it('signs url', async function () {
      const signer = new S3Signer(options);
      const clock = sinon.useFakeTimers(1481095934000);
      const signed = await signer.sign('an evil name with spaces');
      clock.restore();
      expect(signed).to.equal('http://s3-mock-s3-region.amazonaws.com/'
    ...
    it('respects url prefix protocol', async function () {
      const cases = [
      ...
      for (let i = 0; i < cases.length; ++i) {
        ...
        const signed = await signer.sign('an evil name with spaces');
        clock.restore();
        expect(signed).to.equal(expected);
      }
    });
  });
  ...
  describe('Cloud Signer', function () {
    ...
    it('refresh signer token', async function () {
      const signer = new CloudSigner(options);
      ...
      await signer.refreshSignerToken();
    ...
    it('signs url', async function () {
      const signer = new CloudSigner(options);
      const clock = sinon.useFakeTimers(1481095934000);
      sinon.stub(signer, 'refreshSignerToken', async function () {
      ...
      const signed = await signer.sign('index.html');
      clock.restore();
      expect(signed).to.equal('http://mock-cloud-asset.dev/'
